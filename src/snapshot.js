...
const distribute_tokens = ( finish ) => {
  ...
  const maybe_reclaim_tokens = ( registrant ) => {
    ...
    if(reclaimable_transactions.length) {
      ...
      if( reclaimable_transactions.length > 1 ) 
        log("info", `Reclaimed ${ reclaimed_balance.div(WAD).toFormat(4) } CPS from ${reclaimable_transactions.length} tx for ${registrant.eth} => ${registrant.cps}`)
    }
  ...
  const status_log = () => {
    if(index % UI_SHOW_STATUS_EVERY == 0 && index != 0) debug.refresh(), 
      ...
      log("success",  `---------------------------------`),
      log("success",  `Unclaimed:     CPS ${debug.distribution.$balance_unclaimed.toFormat(4) }`),
      log("success",  `In Wallets:    CPS ${debug.distribution.$balance_wallets.toFormat(4)}`),
      log("success",  `Reclaimed:     CPS ${debug.distribution.$balance_reclaimed.toFormat(4)}`),
      log("success",  `Total:         CPS ${debug.distribution.$balances_found.toFormat(4)}`),
      log("error",    `Rejected:      CPS ${debug.rejected.$total.toFormat(4) }`),
      log("error",    `Reclaimable:   CPS ${debug.distribution.$balance_reclaimable.toFormat(4) }`),
      log("error",    `Not Included:  CPS ${debug.distribution.$balances_missing.toFormat(4) }`),
    console.groupEnd()
  }
...
const exporter = ( callback ) => {
  ...
  output.reclaimable = get_transactions_reclaimable().map(      tx => { return { eth: tx.eth, tx: tx.hash, amount: formatCPS(web3.toBigNumber(tx.amount).div(WAD)) }                               } )
  output.reclaimed   = get_transactions_reclaimed().map(        tx => { return { eth: tx.eth, cps: tx.cps, tx: tx.hash, amount: formatCPS(web3.toBigNumber(tx.amount).div(WAD)) }                  } )
  output.snapshot    = get_registrants_accepted().map(  registrant => { return { eth: registrant.eth, cps: registrant.cps, balance: formatCPS(registrant.balance.total) }                          } )
  output.rejects     = get_registrants_rejected().map(  registrant => { return { error: registrant.error, eth: registrant.eth, cps: registrant.cps, balance: formatCPS(registrant.balance.total)}  } )
